...
public class RewriterUtils
{
    ...
    public static String generateValuesQuery(Db2ResultSet db2ResultSet)
    {
        List<List<Object>> rows = db2ResultSet.getResult().stream().filter(row -> !row.isEmpty()).collect(Collectors.toList());
        List<Column> columnNames = db2ResultSet.getColumns();
        if (rows.isEmpty()) {
        ...
        if (columnNames.size() == rows.get(0).size()) {
            ...
            // Do not fail the query here, just skip ` AS t (columnName, ....) ` part of the query, as
            // not showing column names is better than showing incorrect information.
            log.warn("Malformed Db2ResultSet : Column metadata reports [%d] as the column count while row 0 had [%d] columns",
                    columnNames.size(), rows.get(0).size());
            if (log.isDebugEnabled()) {
                log.debug("Column names in metadata: [%s] for query: [%s]", Joiner.on(",").join(columnNames), db2ResultSet.getQueryExecuted());
            }
        }
        sb.trimToSize();
        return sb.toString();
