        ...
        implements QueryRewriterProvider, QueryRewriter
{
    ...
    public QueryRewriterOutput rewriteSQL(QueryRewriterInput queryRewriterInput)
    {
        ...
        if (query.contains("ExecuteWxdQueryOptimizer")) {
            ...
            try {
                Db2ResultSet queryResult = executeOptimizerPlus.callExecuteOptimizerPlus(queryRewriterInput.getQuery());
                rewrittenQuery = RewriterUtils.generateValuesQuery(queryResult);
                ...
                log.debug("Pass through query results returned by DB2 is rendered as : " + rewrittenQuery);
                // Replace the original query with the rewritten query to avoid parsing the ExecuteWxdQueryOptimizer passthrough syntax
                query = rewrittenQuery;
            }
            catch (Exception e) {
                throw new PrestoException(PASS_THROUGH_ERROR_CODE, e);
            }
            return new QueryRewriterOutput.Builder()
                    .setRewrittenQuery(rewrittenQuery)
                    .setOriginalQuery(query)
                    // while executing pass through, we do not want user to see the generated values query.
                    .setUserVisibleQuery(query)
    ...
    @VisibleForTesting
    void setByPassAdminCheck()
    {
        this.byPassAdminCheck = true;
    }
