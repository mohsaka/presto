...
public class ExecuteOptimizerPlus
{
    ...
    public Db2ResultSet callExecuteOptimizerPlus(String execOptPlusQuery)
    ...
    private Db2ResultSet executeCallStatements(String query)
            throws PrestoException
    {
        ...
        try {
            ...
            if (conn == null) {
                return disabledOptimizerPlugin(resultList, query);
            }
            ...
            while (true) {
                if (result) {
                    ...
                    for (int i = 1; i <= columnCount; i++) {
                        ...
                        columns.add(column);
                        log.debug("OPT+ added column: [%s]", column.getName());
                    }
                    while (rs.next()) {
                        ...
                        for (int i = 1; i <= columnCount; i++) {
                            ...
                            row.add(value);
                            log.debug("OPT+ added row [%s]", value);
                        }
                        resultList.add(row);
                    }
                }
            ...
            return new Db2ResultSet(resultList, columns, query);
        }
    ...
    private Db2ResultSet disabledOptimizerPlugin(List<List<Object>> resultList, String query)
    {
        List<Column> columns = new ArrayList<>();
        List<Object> row = new ArrayList<>();
    ...
    private Db2ResultSet executeCallExplainFormat(String query)
    {
        List<List<Object>> resultList = new ArrayList<>();
        List<Column> columns = new ArrayList<>();
        Connection conn = null;
    ...
    private Db2ResultSet executeStatements(String query)
    {
        List<List<Object>> resultList = new ArrayList<>();
        List<Column> columns = new ArrayList<>();
        Connection conn = null;
    ...
    public static class Db2ResultSet
    {
        private List<List<Object>> result;
        private List<Column> columns;
        private String queryExecuted;

        public Db2ResultSet(List<List<Object>> result, List<Column> columns, String queryExecuted)
        {
            this.result = result;
