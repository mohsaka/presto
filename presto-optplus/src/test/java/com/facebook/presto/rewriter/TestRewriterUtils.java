...
import com.facebook.presto.rewriter.optplus.ExecuteOptimizerPlus.Db2ResultSet;
...
import static com.facebook.presto.rewriter.optplus.RewriterUtils.generateValuesQuery;
...
public class TestRewriterUtils
{
    ...
    public void testGeneratedValuesQueryWithQuotes()
    {
        ...
        row.add("Testing escape 'escaped string'. ");
        row.add(null);
        rows.add(row);

        String expected = " SELECT * FROM (VALUES ('Testing escape ''escaped string''. ', 'null'))" +
                " AS t (\"col1\", \"col2\")";
        List<Column> columns = unmodifiableList(asList(new Column("col1", VarcharType.VARCHAR),
                new Column("col2", VarcharType.VARCHAR)));
        assertEquals(generateValuesQuery(new Db2ResultSet(rows, columns, "")), expected);
    }
    ...
    @Test
    public void testGeneratedValuesQueryWithEmptyResultSet()
    {
        String expected = "SELECT 0 LIMIT 0";
        assertEquals(generateValuesQuery(new Db2ResultSet(emptyList(), emptyList(), "")), expected);
        // empty rows check.
        List<List<Object>> rows = new ArrayList<>();
        ArrayList<Object> row = new ArrayList<>();
        rows.add(row);
    ...
    public void testGeneratedValuesQueryWithEmptyColumnsInformation()
    {
        ...
        assertEquals(generateValuesQuery(new Db2ResultSet(rows, emptyList(), "")), expected);
    }

    @Test
    public void testGeneratedValuesQueryWithEmptyStringValue()
    {
        List<List<Object>> rows = new ArrayList<>();
        List<Column> columns = ImmutableList.of(new Column("string column", CharType.createCharType(2)));
        List<Object> tempRow = new ArrayList<>();
        tempRow.add("");
        rows.add(tempRow);
        ...
        tempRow.add('b');
        rows.add(tempRow);
        String expected = " SELECT * FROM (VALUES (''), ('b')) AS t (\"string column\")";
        assertEquals(generateValuesQuery(new Db2ResultSet(rows, columns, "")), expected);
    }
