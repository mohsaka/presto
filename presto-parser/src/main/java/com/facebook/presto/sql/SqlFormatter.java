            ...
            extends AstVisitor<Void, Integer>
    {
        ...
        protected Void visitTableArgument(TableFunctionTableArgument node, Integer indent)
        {
            ...
            node.getOrderBy().ifPresent(orderBy -> {
                builder.append("\n");
                append(indent, formatOrderBy(orderBy, parameters));
            });
                ...
                builder.append(node.getDescriptor().get().getFields().stream()
                        .map(field -> {
                            ...
                            if (field.getType().isPresent()) {
                                formattedField = formattedField + " " + formatExpression(field.getType().get(), parameters);
                            }
        ...
        {
            return ExpressionFormatter.formatExpression(identifier, Optional.empty());
        }
