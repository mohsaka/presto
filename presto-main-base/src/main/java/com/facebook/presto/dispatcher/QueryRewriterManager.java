...
public class QueryRewriterManager
{
            ...
            QueryPreparerProvider queryPreparerProvider)
    {
        ...
        if (getQueryRewriterProvider().isPresent()) {
            ...
            return new QueryAndSessionProperties(Optional.ofNullable(rewrittenQuery), Optional.ofNullable(queryRewriterOutput.getOriginalQuery()), systemPropertyOverrides, queryRewriterOutput.getRuntimeStats());
        }
        return new QueryAndSessionProperties(Optional.empty(), Optional.empty(), session.getSystemProperties(), new RuntimeStats());
    }
}
